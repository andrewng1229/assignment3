...
def fit_regression_model(X, y):
    ...
    """
    learning_rate = 0.00015# Pick a better learning rate
    num_epochs = 4001 # Pick a better number of epochs
    input_features = X.size(1) # extract the number of features from the input `shape` of X
    output_features = y.size(1) # extract the number of features from the output `shape` of y
    ...
    for epoch in range(1, num_epochs):
        ...
        if abs(previos_loss - loss.item()) == 0.0001: # Change this condition to stop the training when the loss is not changing much.
            break
